    ...
    <div className="bg-white rounded-[10px] p-6 hover:shadow-md transition-shadow">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-[16px] text-gray-400">{brand} - {type}</p>
        </div>
        <button className="p-1">
          {isFavorite ? (
            <FaHeart className="w-6 h-6 text-red-500" />
          ) : (
            <CiHeart className="w-6 h-6 text-gray-400" />
          )}
        </button>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div>
          <div className="flex items-baseline gap-1">
            <span className="text-[20px] font-bold">${pricePerDay}</span>
        ...
        <div className="flex gap-2">
          <Link href="/pages/cart">
            <button className="p-2 text-[#3563E9] hover:bg-blue-50 rounded-lg transition-colors" aria-label="Add to cart">
              <FaShoppingCart size={30} />
            </button>
          </Link>
          <Link href="/pages/detailcarRent">
            <button className="bg-[#3563E9] text-white px-5 py-2.5 rounded-[4px] hover:bg-blue-700 transition-colors">
              Rent Now
            </button>
          </Link>
        </div>
